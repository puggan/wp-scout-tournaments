<?php
	...
	add_action('admin_menu', 'init_add_game_menu');
	...
	function init_add_game_menu()
	{
		$icon_url = get_template_directory_uri() . "/game/ball.png";
		$menu_slug = 'game';
		$capability = 'edit_pages';

		add_menu_page('Administrera Spel', 'Spel', $capability, $menu_slug, 'game_page', $icon_url, 58);

		add_submenu_page($menu_slug, "Deltagande Lag", "Lag", $capability, $menu_slug . "_teams", "game_team_page");
		add_submenu_page($menu_slug, "Grupper", "Grupp", $capability, $menu_slug . "_groups", "game_group_page");
		add_submenu_page($menu_slug, "Matcher", "Match", $capability, $menu_slug . "_match", "game_match_page");
		add_submenu_page($menu_slug, "Edit Match", "Edit Match", $capability, $menu_slug . "_edit_match", "game_edit_match_page");

		add_submenu_page($menu_slug, "Dommare", "Dommare", $capability, $menu_slug . "_referees", "game_referees_page");
	}
	...
	function game_team_page()
	{
		...
		if(isset($_POST['bulk_button']))
		{
			...
			{
				if(isset($row['action']) AND $row['action'] == 'add')
				{
					if(game_add_team($row))
					{
						echo "<p class='notice'>Lag " . html_encode($row['team_name']) . " registrerad</p>";
					}
					else
					{
						echo "<p class='warning'>Misslyckades registrera Lag " . html_encode($row['team_name']) . "</p>";
					}
				}
			}
		}

		if(isset($_POST['add']['action']) AND $_POST['add']['action'])
		{
			if(game_add_team($_POST['add']))
			{
				echo "<p class='notice'>Lag " . html_encode($_POST['add']['team_name']) . " registrerad</p>";
			}
			else
			{
				echo "<p class='warning'>Misslyckades registrera Lag " . html_encode($_POST['add']['team_name']) . "</p>";
			}
		}
		...
		if(isset($_GET['team_id']) AND $_GET['team_id'])
		{
...
JS_BLOCK;
				...
				foreach($missing_teams as $lead_id => $team)
				{
					...
					echo "<td><input type='text' readonly='readonly' name='bulk[{$lead_id}][team_name]' value='" . html_encode($team->team_name) . "' /></td>";
					echo "<td><input type='hidden' name='bulk[{$lead_id}][class_id]' value='{$team->class_id}' />" . html_encode($classes[$team->class_id]->class_name) . "</td>";
					echo "</tr>";
				}
			...
			{
				echo "<option value='{$class_id}'>" . html_encode($class->class_name) . "</option>";
			}
	...
	function game_add_team($data)
	{
		global $wpdb;

		if(!isset($data['team_name']) OR !$data['team_name'])
		{
			trigger_error("game_add_team() require 'team_name'");
			return FALSE;
		}

		if(!isset($data['class_id']) OR $data['class_id'] < 1)
		{
			trigger_error("game_add_team() require 'class_id'");
			return FALSE;
		}

		// FIXME: buld add, double encodes ' => in database \'
		return $wpdb->insert("game_teams", array('team_name' => $data['team_name'], 'class_id' => $data['class_id']), array("%s", "%d"));
	}

	function game_group_page()
	{
		global $wpdb;

		if(isset($_POST['add']['action']) AND $_POST['add']['action'])
		{
			if(game_add_group($_POST['add']))
			{
				echo "<p class='notice'>Grupp " . html_encode($_POST['add']['group_name']) . " registrerad</p>";
			}
			else
			{
				echo "<p class='warning'>Misslyckades registrera Grupp " . html_encode($_POST['add']['group_name']) . "</p>";
			}
		}
		...
		if(isset($_POST['connect']['action']) AND $_POST['connect']['action'])
		{
			if(set_team_group($_POST['connect']['team_id'], $_POST['connect']['group_id']))
			{
				echo "<p class='notice'>Kopplade '" . html_encode($teams[$_POST['connect']['team_id']]->group_name) . "' till grupp '" . html_encode(
			...
			{
				echo "<p class='warning'>Misslyckades koppla '" . html_encode($teams[$_POST['connect']['team_id']]->group_name) . "' till grupp '" . html_encode(
						$groups[$_POST['connect']['group_id']]->group_name
					) . "'</p>";
			}
		}
		...
		{
			echo "<option value='{$team_id}'>" . html_encode($team->team_name) . " - " . ($team->group_name ? html_encode($team->group_name) : "?") . " - " . html_encode($team->class_name) . "</option>";
		}
		echo "</select></label><br />";
		...
		{
			$selected = (isset($_POST['connect']['group_id']) AND $_POST['connect']['group_id'] == $group_id);
			echo "<option value='{$group_id}'" . ($selected ? " selected='selected'" : "") . ">" . html_encode($group->group_name) . "</option>";
		}
		echo "</select></label><br />";
	...
	function game_add_group($data)
	{
		global $wpdb;

		if(!isset($data['group_name']) OR !$data['group_name'])
		{
		...
		return $wpdb->insert("game_groups", array('group_name' => $data['group_name'], 'class_id' => $data['class_id']), array("%s", "%d"));
	}

	function set_team_group($team_id, $group_id)
	{
		global $wpdb;

		return $wpdb->update("game_teams", array('group_id' => $group_id), array('team_id' => $team_id), array("%d"), array("%d"));
	}

	function game_match_page()
	{
		global $wpdb;
...
ORDER BY IF(game_teams.group_id IS NULL, 0, 1), game_teams.class_id, game_teams.group_id, game_teams.team_name';
		...
		if($matcher)
		{
			foreach($matcher as $match)
			{
				...
				echo "<td>" . htmlentities($match->field_name) . "</td>";
				echo "<td>" . htmlentities($match->referee_name) . "</td>";
				echo "<td>" . htmlentities($match->match_status) . "</td>";
				echo "</tr>";
			}
		}
		else
		...
		foreach($teams as $team)
		{
			echo "<option value=\"{$team->team_id}\">" . htmlentities($team->team_name) . "</option>";
		}
		echo "</select><br />";
	...
	function game_edit_match_page()
	{
		...
		echo "<form action=\"#\" method=\"post\">";
		echo "<label><span>ID: </span>" . htmlentities($match->match_id) . "</label><br />";
		...
		foreach($teams as $team)
		{
			...
			{
				echo "<option value=\"{$team->team_id}\" selected=\"selected\">" . htmlentities($team->team_name) . "</option>";
			}
			else
			{
				echo "<option value=\"{$team->team_id}\">" . htmlentities($team->team_name) . "</option>";
		...
		echo "</select></label>";
		echo "<input name=\"home_team_description\" value=\"" . htmlentities($match->home_team_description) . "\" /><br />";
		...
		echo "</select></label>";
		echo "<input name=\"away_team_description\" value=\"" . htmlentities($match->away_team_description) . "\" /><br />";
		echo "<label><span>Plan: </span><input name=\"field\" value=\"" . htmlentities($match->field_id) . "\" /></label><br />";
		echo "<label><span>Time: </span><input name=\"time\" value=\"" . htmlentities($match->match_time) . "\" /></label><br />";
		echo "<label><span></span><input type=\"submit\" value=\"Save\" /></label><br />";
		echo "</form>";
	}

	function game_referees_page()
	{
		global $wpdb;

		if(isset($_POST['add']['action']) AND $_POST['add']['action'])
		{
			$result = $wpdb->insert("game_referees", array('referee_code' => $_POST['add']['referee_code'], 'referee_name' => $_POST['add']['referee_name']), array("%s", "%s"));
