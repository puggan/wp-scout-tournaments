...
(static function (Database $database, string $siteName) {
    ...
    foreach ($db_matches as $match) {
        ...
        if ($match['match_time']) {
            ...
            if ($match['match_status'] && $match['match_status'] !== 'QUEUE') {
                $breaks[] = ['break_name' => "Match {$match['match_id']}", 'break_from' => $match['match_time'], 'break_to' => $match_end_time];
            } else {
    ...
    while ($todo_matches) {
        ...
        usort($todo_matches, static function ($a, $b) {
...
// 		echo "<pre>Match: " . print_r($next_match, TRUE) . "</pre><hr />";
    ...
    usort($breaks, static function ($a, $b) {
        return strcmp($a['break_to'], $b['break_to']);
    });
...
// 	echo "<pre>" . print_r($match_order, TRUE) . "</pre>";
    ...
    while ($match_order) {
        ...
        foreach ($breaks as $break) {
            if ($break['break_from'] < $next_match_end_time && $next_match_time < $break['break_to']) {
