...
<div id="wrapper" class="hfeed">
    <div class="skip-link"><a class="assistive-text-main" href="#main" title="<?php esc_attr_e( 'Skip to content', 'scout' ); ?>"><?php _e( 'Skip to content', 'scout' ); ?></a></div>
	<div class="w1">
...
//					get_search_form();
				...
				<nav class="nav-sub">
					<?php wp_nav_menu( array( 'theme_location' => 'top', 'menu_class' => 'sub-nav', 'fallback_cb' => false ) ); ?>
				</nav>
			</div><!-- .holder -->
			...
			<nav id="access" role="navigation">
				...
				<?php wp_nav_menu( array( 'theme_location' => 'main', 'menu_id' => 'nav', 'container_class' => 'menu-main-container', 'fallback_cb' => false ) ); ?>
			</nav><!-- #access -->
<?php
/*
			<form method="get" action="<?php echo esc_url( home_url( '/' ) ); ?>" class="mobile-form">
				<fieldset>
					<span class="text"><span><input type="text" name="s" value="<?php echo __('Search on', 'scout'); echo ' '.$_SERVER['HTTP_HOST']; ?>" /></span></span>
				</fieldset>
			</form>
*/
?>
            ...
            $fields = get_option('scout_footer_options');
				if(!empty($fields)) {
					if(!empty($fields['contact'])) {
						if(!empty($fields['contact']['contact_page'])) echo '<a href="'.$fields['contact']['contact_page'].'" class="contact-link">'.__('Contact', 'scout').'</a>';
					}
				}
			?>
		</header><!-- #header -->
    ...
    <?php if( !is_front_page() && !is_search() ) {
		...
		<?php
			...
			if( get_post_type() == 'post') {
				if( is_author() || is_category() || is_tag() || is_date() || is_home() || is_singular( 'post' ) ) {
					get_template_part('sidebar', 'blog');
				}
			} else if( !is_front_page() && !is_404() && $menuid =! "" && !is_page_template('templates/page-fullwidth.php')) {
